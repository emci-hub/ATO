-- Qualify auth table columns in pause/unpause/delete_branch.
-- RETURNS TABLE (user_id ...) made bare `user_id` ambiguous.
...
create or replace function public.unpause_branch(p_user_id uuid)
returns table (user_id uuid, handle text)
language plpgsql
security definer
set search_path = public
