-- Wave 24 — fix re-save data loss on recovery_style.
-- wave23_optional_intake.sql's complete_signup re-save branch did a plain
-- assignment (`recovery_style = p_recovery_style`) instead of coalescing like
-- every other optional field in the same branch. Now that the app no longer
-- collects recovery_style (question removed from Nine/Eight Quick Taps), it
-- never sends a real value, so any already-onboarded user hitting the
-- re-save path would have their existing recovery_style silently nulled.
-- This restores the coalesce so historical values on that column survive.
-- Only this one line changes; everything else matches wave23 verbatim.
