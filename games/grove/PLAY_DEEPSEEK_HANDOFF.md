...
| C | ✅ 2026-09-09 | **Bosses + soft type match + previews + Avatar star.** Main 9/10/19/20 boss bands (data `waves.json`: fat HP + `type_tint` + size + Runner pack + optional HP-threshold `burst` enrage that spawns runners — no new skill primitive). Soft type match Tide/Ember/Root/Spark: equipped Power `type_tag` == cycle tint → +20% board power (`typeMatchBonus`), mismatch neutral; chart + `?` on Defend setup/live. Wave + drop preview on setup/band select (Farmable vs Unique/Owned via `DropTable`; `uniques[]` in store). Avatar star: Final clear 25% once/cycle (`avatar_star_rolled_cycle`), pity on 3rd Final that cycle; spend token → +★, +3% base wave_power. Dive Power weights trimmed to ~20% (Dive = volume lane; Defend owns uniques/tint). Store v13 (`avatar_stars`, `avatar_star_tokens`, `avatar_star_rolled_cycle`, `final_clears_this_cycle`, `uniques[]`, `cycle_tint`). Dev kit (Calgary1! intact): Jump to Final (Main 20), grant/spend star token, cycle tint, dump drops for band. No Bound Boss / GS skip (D/E). |
| D | ✅ 2026-09-09 | **GS + Skip-to-even** (`engine/gear-score.ts`, no store bump). `GS = floor(100 × wave_power_eff × (1 + 0.02·avatar_level) × (1 + 0.03·avatar_stars))` after soft-caps — type match is combat only, never GS. `recommended_GS(wave) = base[wave] × cycle_power`; base[wave] derived from the §9 wave formula (count × hp, Trial w1 = 100, damped count tail). Skip offer when `GS ≥ 1.25 × recommended(next)` (`skipGsThreshold` Tune): fast-forwards normal waves only, STOPS at boss bands 9/10/19/20 (`planSkipToEven` + `skipCampaignToEven`); pays ~45% tokens/XP per skipped wave (`skipPayFraction` Tune 0.45, no daily-cap touch) + ONE commons-only skip crate per batch (`drop_skip_crate` — common Looks/Powers only: never uniques/tint gear/Avatar star/boss fragments). Milestones 5/10/25 fire on lifetime crossings (skip advances `lifetime_waves_cleared`). Seat moves so farm bands unlock below the stop. Defend setup card shows "Your GS · Wave wants ~" + the skip offer; skip toast lists reduced tokens/XP + crate + milestones. Dev kit (Calgary1! intact): Set GS high (level 200 ★5 + ★5 Powers), Force skip offer (overgear → Trial 1), Dump GS vs recommended. Smoke: overgeared skip stops at Scout (Main 9), never auto-runs 9/10/19/20. No Bound Boss (E). |
