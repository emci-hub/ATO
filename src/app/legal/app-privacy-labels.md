...
| Other User Content | Yes | Linked | No | App Functionality, Product Personalization | Chat messages; Sage messages; ME free text (`show_up`, `knocks_you_off`, `morning_cue`, `talk_style`, `facts`, typed `city` for Around, IANA `timezone`); pixel `recipe`; Check read/do text. Gemini receives a prompt derived from ME + recent checks, not the full chat history. |
