...
function knockLine(chip: string): string {
  switch (chip) {
    case 'sleep':
      return "Sleep is what you said knocks you off, and it showed up in this week's Checks. Keep today's ask tiny so tonight can still start on time.";
    case 'workload':
      return "Workload showed up in this week's Checks — the thing you already named. Today's Do is a single slice of the pile, then a stop.";
