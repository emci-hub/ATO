...
**Decision: Wave 1.5 and Wave 3 both start now, in parallel — deliberately not following the plan's original stage order.** Wave 2 Stage 2 ("I'm going") is now live, so Night wall is unblocked. Stage 9 first pass is in; identity chips are editable in Settings; Stage 11 optional fast-entry is in (all 15 axes; direct vs inferred; `last_touched`); Talk output fence is in. Next Wave 1.5 box: Stage 12 (Sage coaching content). ATO_PLAN_v2.md is a working reference — Aug 28 design (Explore, three-path intake, Reload) and the Grok review locks are recorded there; extra axes + source rank + Talk fence have shipped.
