...
**Decision: Wave 1.5 and Wave 3 both start now, in parallel — deliberately not following the plan's original stage order.** Wave 2 Stage 2 ("I'm going") is now live, so Night wall is unblocked. Stage 9 first pass is in; identity chips are editable in Settings; Stage 11 optional fast-entry is in (9 of 15 axes); next Wave 1.5 box: Stage 12 (Sage coaching content). ATO_PLAN_v2.md is a working reference — Aug 28 design (Explore, extra axes, three-path intake, Reload) is recorded there, not built.
