...
- **Floor requirements (built):** Sentry JS+native SDK in the app (You-tab test buttons). App Privacy nutrition labels + `PrivacyInfo.xcprivacy` match (`NSPrivacyTracking = false`). Sage labeled coach on Talk/Home/morning push/widget. Talk model calls rate-limited per user server-side (`claim_ai_call`, 20/day 200/month). Sentry DSN still to be pasted; native crash lands after the next EAS build.

## Roster
- Assistant (Drake) — active
- Builder (Cursor) — active, Stages 1–7 + Stage 8 Apple handoff + Stage 8 invite/referral handoff + Stage 8 push/widget + Stage 8 floor-requirements sweep shipped; owns docs/ME.md, docs/NOW.md, docs/ATO_PLAN_v2.md, and docs/BUSINESS.md. Updates to any of those four are committed **and pushed** together — never left local-only.
