...
- **Plan framing:** ATO_PLAN_v2.md is a working reference, not a locked spec. Stage 11's first 9 trait axes are in; Talk output fence is in. Extra axes / Explore / three-path intake / Reload are decided in that file, not built. Aug 28 Grok review locks (Explore combine, Does-Sage-know-you, completeness split, direct-vs-inferred sources, Talk fence, soft-ask budget, `last_touched`) live in the Understanding spec. Next Wave 1.5 box remains Stage 12. Crisis / coach-label / diagnosis-avoidance / App Store floor items in the plan are compliance-grounded.
