...
- **Home:** Stage 1 fake poster / fake card / "open box" chrome is off the production Home route. No card → honest empty, not fixture copy. Live pixel is the small top-right nav companion, not a large centered face. Tap plays a short coherent mood. A small all-time Checks chip sits on Home (presence-tier glow, all five appearance modes). You-tab poster is name / handle / visibility / QR (no large pixel). Sage is `Sage · coach` on Talk/Dawn/widget/push and on Home except Quest, where the Home card reads `Sage · npc`. Home may show a Nudge card from a real recent signal; Circle/widget/push never do.
