...
- **Do:** Apple Developer Program already enrolled (see M0). Remaining: Individual vs Organization still open; wait on Beta App Review; **submit binary 10** (`1d0d1041`) to TestFlight — not 8 or 9; Sentry native symbolication on event `e7bed112` is unconfirmed. $99 USD/yr already in play. **Wave 1.5 and Wave 3 start now in parallel** (intentional deviation; Night wall must not surface to testers until Wave 2 Stage 2 is live). Stage 9 first pass (intake core + Day 1 bank wiring) is in; next Wave 1.5 box is Stage 11 (optional fast-entry). AI capacity hardening stays a public-launch backlog item.
