...
- **Plan framing:** ATO_PLAN_v2.md is a working reference, not a locked spec. All 15 trait axes are in, with direct-vs-inferred `trait_sources` and `last_touched`. Talk output fence is in. Explore / three-path intake / Reload are decided in that file, not built. Aug 28 Grok review locks (Explore combine, Does-Sage-know-you, completeness split, Talk fence, soft-ask budget) live in the Understanding spec. Next Wave 1.5 box remains Stage 12. Crisis / coach-label / diagnosis-avoidance / App Store floor items in the plan are compliance-grounded.
