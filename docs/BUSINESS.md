...
- **Do:** Apple Developer Program already enrolled (see M0). Remaining: Individual vs Organization still open; wait on Beta App Review; **submit binary 10** (`1d0d1041`) to TestFlight — not 8; Sentry native symbolication on binary 8 is untested (build 6 native frames stay unsymbolicated). $99 USD/yr already in play. Wave 1.5 (Understanding & Delight, Stages 9–14) can build during friends-only TestFlight and is not blocked on public App Store readiness — Stage 9 (intake core) is next once those Stage 8 floor items close. AI capacity hardening stays a public-launch backlog item, not Wave 1.5.
