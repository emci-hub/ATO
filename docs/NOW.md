...
**Explore is its own tab (Sep 1, 2026).** The Sage tab is now clean chat only — 8-ball + conversation, nothing else. **Explore is a real bottom tab**, opened directly — no "Explore ›" button in Sage. The Explore tab holds: the axis + category summaries (`IntakeSettings` "How you show up", `TraitBandsFold` "How you tend to move", `FullProfileFold` "How you're currently leaning", and `CategoriesFold` "Categories" at full detail — bars/maps, concept explainers, weekly spotlight), "What Sage remembers" (`SageFactsCard`), The Story, the Sage title card, Notes insight spend, and the Explore observation bubbles with "Did this land?" reactions. "N of 16 settled" lives on the Explore tab header (no longer on Sage).
...
**Facts list is in.** Summary row next to "How you show up" on Explore: "Sage remembers N things" or "Nothing yet". Tap opens "What Sage remembers" — read/delete only; teaching stays Chat "Teach Sage this".
