...
**Library copy is in.** Ten written-once entries in `src/app/copy/library.md` (same storage as `first_cards.md`): domain Sleep / Workload / Conflict / Communication / Health / Money, plus Self-Determination Theory, growth mindset, locus of control, self-efficacy. Academic / public-health sources only. Sage is not wired — **For Sage** lines are fence-clean paraphrases for a later box.
