...
4. ✅ **Floor requirements sweep — DONE.** Sentry JS+native SDK wired (`@sentry/react-native` ~7.2, Expo plugin, Metro, `Sentry.wrap`, You-tab **Test crash reporting**). App Privacy nutrition labels + `PrivacyInfo.xcprivacy` filled from what ATO actually collects and cross-checked against `app.json` `ios.privacyManifests` (`npx tsx scripts/floor-check.ts` 8/8). Sage labeled coach on Talk, Home, morning push, and the widget — not implied to be a person in the UI. Per-user Talk cap is server-side (`claim_ai_call`, `ai_usage`, caps on `app_config` 20/day 200/month); hitting it shows "Sage's out of things to say for today, back tomorrow". `npx tsx scripts/quota-check.ts` 5/5; SQL proved cap=1 allows the first claim and denies the second. **Sentry dashboard confirmation still needs a DSN:** create a free Sentry project, set `EXPO_PUBLIC_SENTRY_DSN` in `.env.local` (and EAS env), tap **JS error** on You. Native crash capture needs the next EAS binary (this machine cannot run an iOS native crash).
...
- Stage 8 handoff #4 (floor requirements) — Sentry/privacy/coach/quota built and checked (`floor-check` 8/8, `quota-check` 5/5, voice 24/24 including quota deny). Sentry dashboard landing still needs `EXPO_PUBLIC_SENTRY_DSN`; native crash needs the next EAS build.

## Left
Stage 8 (TestFlight) — handoffs 5–6, see sequenced list under On
...
## Next 15 min
Stage 8 handoff #5: legal + landing copy (you, not Cursor), then EAS → TestFlight.
