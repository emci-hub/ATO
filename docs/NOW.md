...
**Latest production OTA:** `b77e49ce-9605-4e04-aeb7-40b0ccb8537c` (commit `34ce429`, runtime 1.0.0, ios+android) — trait-bands unification: "How you tend to move" reads the same report-track EWMA as Full Profile, so one axis shows one number across both folds. Published Sep 3, 2026. Binary 10+. Supersedes `b0bdd310` (commit `74242a6`, 5-slot bottom nav wave37 + poster restyle).
...
**EAS Update (OTA) is live as of binary 10.** Devices on binary 10+ receive JS via `eas update`. Devices on binary 8 or earlier cannot. Latest production JS: group `b77e49ce-9605-4e04-aeb7-40b0ccb8537c` (commit `34ce429`, trait-bands unification). **OTA published Sep 3, 2026.** Prior groups (`c897410d`, `d5332b8b`, `11d99ff3`, `b0bdd310`) are superseded.

## Done
**Trait-bands unification OTA (commit `34ce429`, Sep 3, 2026).** Group `b77e49ce`. "How you tend to move" (`TraitBandsFold`) now reads the same `trait_tracks` report EWMA as Full Profile / Categories / Title / Story / the settled header, falling back to the raw `me` column when no report track exists — one canonical trait number per axis everywhere. Client-only display fix, no schema change. **OTA published Sep 3, 2026.** 100% production channel — no staged rollout. Supersedes `b0bdd310`.
...
## Next 15 min
Work through `docs/ATO_DEVICE_TESTS.md` in full on a real device with **binary 10** installed (it pulls the latest OTA `b77e49ce-9605-4e04-aeb7-40b0ccb8537c`). Every box's checklist, one sitting. Bring back anything that fails. Once that pass is clean, Stage 8 handoff #2: invite/referral gate (Auth + ME). Open items that are not the device pass: dev-test auto-login + Legends persona strip verification, Gut Call regression, Live Talk failure, closing the `emci2` root-handle gap, Claude/Grok Supabase secrets if those providers will be used.
