...
## Next 15 min
Waiting on Apple: Beta App Review for the Friends external testing group (build 6). EAS production **binary 8** is cut (`d40e57a9`) — submit to TestFlight when ready. App icon swap still queued. Confirm Sentry maps after install. Do not start public-release items. Once remaining Stage 8 floor items close, **Stage 9 (intake core)** is next.
