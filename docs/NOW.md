...
**Latest production OTA:** `9855305a-2d09-4184-8efb-bb1e8b741490` (commit `96f61b8`) — friend-voice style checklist across all 5 generation surfaces
...
**Nav bar is fixed.** Tabs are Home / Sage / Around / Circle (appears only after a connection) / You. **There is no reorder, "More" tab, or nav customization** — that feature does not exist in the code. Home and Sage cannot be hidden.
...
**EAS Update (OTA) is live as of binary 10.** Devices on binary 10+ receive JS via `eas update`. Devices on binary 8 or earlier cannot. Latest production JS: group `9855305a-2d09-4184-8efb-bb1e8b741490` (`96f61b8`, friend-voice style checklist). **OTA published Aug 31, 2026.** Prior group `04c91f24-7c55-42d1-bbb0-a1f8a4859eeb` (`0f4dc3a`, Wave 22 + warmer Story copy) is superseded.
...
**Friend-voice style checklist OTA** (commit `96f61b8`). Group `9855305a`. Shared six-rule style checklist + approved anchors across Dawn Read, Explore, Title, Category, Story. **OTA published Aug 31, 2026.** 100% production channel — no staged rollout.
