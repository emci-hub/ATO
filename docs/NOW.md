...
**Latest production OTA:** `5999d5d4-06a2-46e9-bc7e-5ab1938a8711` (commit `34f45a5`) — Explore as its own full screen; Sage tab is clean chat
...
**Explore as its own full screen OTA** (commit `34f45a5`). Group `5999d5d4-06a2-46e9-bc7e-5ab1938a8711`. Sage tab is clean chat again (8-ball + conversation + an "Explore ›" header button). The pushed `/explore` screen holds the title card, Categories at full detail (`CategoriesFold`), The Story, Notes insight spend, and the observation bubbles. Drops the unused `ExplorePinnedCategories` component. **OTA published Aug 31, 2026.** 100% production channel — no staged rollout. Binary 10+ picks it up on launch.
