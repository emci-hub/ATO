...
- **Stage 8 handoff #6 (EAS → TestFlight)** — build 6 shipped and installed on a real device. Beta App Review submitted for the Friends external testing group (pending as of Aug 26, 2026). **EAS production binary 8** cut Aug 27 (`d40e57a9-c9c1-4db5-8912-0ae2403c40b0`, git `2a0732c`, message labeled "Build 7"; number 7 was consumed by a failed credential attempt). Includes appearance, Around, age field, Home fixture strip. **Not submitted to TestFlight yet** (no `eas submit`). App icon swap is not in this binary.
- **App icon swap — wired in code.** Pack was on the Desktop (`C:\Users\lil_e\Desktop\AppIcons`), not in the repo; copied `AppIcon.icon/` into `assets/`. `app.json` `ios.icon` is `./assets/AppIcon.icon` (SDK 54 Icon Composer / iOS 26). Same `.icon` format as the previous Expo template (`assets/expo.icon`), which already shipped TestFlight builds — no SDK incompatibility. `npx expo prebuild --clean` cannot generate an iOS project on Windows (Expo skips iOS codegen off macOS/Linux); EAS/macOS prebuild is the native confirm. Fallback if Icon Composer ever fails: point `ios.icon` (or top-level `icon`) at flattened RGB `assets/AppIcon-fallback-1024.png`. Pack `appiconset/1024.png` and `appstore.png` were RGBA; flattened to RGB by compositing onto opaque white. `AppIcon.icon/Assets/icon.png` was already RGB. Not in binary 8 — next EAS production cut.

## Left
- **EAS binary 8 → TestFlight:** run `eas submit` (not done). Then install on device: appearance picker, Around, age field, Home without fake chrome.
- **Still queued (not in binary 8):**
  - App icon — **wired**, not in this binary. Next EAS production cut picks up `ios.icon` → `./assets/AppIcon.icon`.
  - Sentry source-map confirmation — auto-upload was on for this production cut; confirm maps (and a native event) in `ato-app` after the binary is on a device
...
## Next 15 min
Waiting on Apple: Beta App Review for the Friends external testing group (build 6). EAS production **binary 8** is cut (`d40e57a9`) — submit to TestFlight when ready. App icon is wired in code (`./assets/AppIcon.icon`); ships on the next EAS cut, not binary 8. Confirm Sentry maps after install. Do not start public-release items. Once remaining Stage 8 floor items close, **Stage 9 (intake core)** is next.
