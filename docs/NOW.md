...
- **App icon swap — wired in code.** Pack was on the Desktop (`C:\Users\lil_e\Desktop\AppIcons`), not in the repo; copied `AppIcon.icon/` into `assets/`. `app.json` `ios.icon` is `./assets/AppIcon.icon` (SDK 54 Icon Composer / iOS 26). Same `.icon` format as the previous Expo template (`assets/expo.icon`), which already shipped TestFlight builds — no SDK incompatibility. Fallback if Icon Composer ever fails: point `ios.icon` (or top-level `icon`) at flattened RGB `assets/AppIcon-fallback-1024.png`. Not in binary 8; included in the OTA production cut below.
- **EAS Update / OTA — wired.** `expo-updates` `~29.0.20` installed. `eas update:configure` set `updates.url` = `https://u.expo.dev/03d53c8c-91ae-4993-aba9-edf0b2eefd1e`. Default `runtimeVersion` policy was `appVersion`; replaced with a **fixed string** `"1.0.0"` for explicit compatibility boundaries (bump this when native code changes). Production channel exists (`01a043c9-fde4-7000-87b5-c8263fae3d71`) and is set on the `production` build profile in `eas.json` (`"channel": "production"`). Preview and development channels created to match those profiles. After the next production binary, JS/UI/backend-only changes go out with `eas update`, not another native build.

## Left
- **EAS binary 8 → TestFlight:** skip — do not submit 8. The OTA + icon production cut is the one to submit.
- **Still queued:**
  - Sentry native symbolication — **untested** on binary 8 (no crash events, no `1.0.0+8` release). Re-check on the OTA binary after it is on a device.
  - Submit the OTA + icon production binary to TestFlight (`eas submit`) once that cut finishes.
...
## Next 15 min
Waiting on Apple: Beta App Review for the Friends external testing group (build 6). Do not submit binary 8. The next production cut (OTA + real app icon) is the one to submit. Sentry native symbolication on build 8 is untested. Do not start public-release items. Once remaining Stage 8 floor items close, **Stage 9 (intake core)** is next.
