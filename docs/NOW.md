...
## Done
**More sheet "You" tap fixed (commit `3b3a1c6`, Sep 1, 2026 — code only, not yet OTA'd).** The `76483a7` More-sheet fix moved hidden `TabTrigger`s for tabs parked in More, but placed them as siblings after `</TabList>` instead of inside it — expo-router/ui's `Tabs` only registers `TabTrigger`s that are descendants of `TabList`, so any reorderable tab moved into More (e.g. "You") silently stopped navigating on tap. Hidden triggers now render inside `TabList` (zero-sized via a `width:0/height:0` style) so they stay registered regardless of bar/More placement. `nav-check.ts` still 14/14; no visual change to the bar.
