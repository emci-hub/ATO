...
- `question_bank_pool` — shared, not user-scoped: `id, axis (16-axis check), category, prompt, options jsonb, source ('authored'|'ai'), created_at, times_served int default 0`. Seed migration: bulk-insert `QUESTIONS_BANK`'s 50 items as `source='authored'` rows (dedup key on prompt text). Growth target ~100 total is a soft cap enforced in application code (`addToBankPool`, below), not a DB constraint — new AI-authored items only get added when a needed axis is genuinely short relative to its tiered target, not on every generation.
...
- `src/lib/questions/reroll.ts` — `rerollQuestion(userId, questionItemId)`: spends 1 ATO token (§5), inserts into `question_bank_reroll_exclusions` for whichever bank item the outgoing question came from (if it came from the bank — an AI-generated-on-the-fly question with no bank row is simply not re-servable to that user again via its own `question_items` history, no exclusion row needed), draws one replacement via `fetchBankCandidates` (bank-first, AI-fallback, same as initial round composition), and writes the replacement into the same `question_items` row/slot.
...
- Save path: `saveOngoingRoundBatch(userId, drafts)` → new `question_packs` row (`kind='ongoing_round'`) + 25 `question_items` rows, mirroring `category-batch-store.ts`'s existing save pattern. Rows drawn from the bank should carry a `question_bank_item_id` reference (new nullable FK column on `question_items`) so `reroll.ts` knows which bank row to exclude — AI-generated-fresh rows leave it null.
...
### Claude's own open questions after this update (not yet answered — flagging rather than guessing)

- **Q8 — Does the post-50 reroll's permanent exclusion apply per-user only, or per-user-per-question-instance?** As written, `question_bank_reroll_exclusions` keys on `(user_id, question_bank_item_id)` — once a user rerolls away a bank question, that SAME bank item can never be drawn for them again, ever, even in a completely unrelated future round. Confirm this is the intended strength (vs., say, only excluding it from resurfacing for N days/rounds) — the plan says "never resurface... permanent," which I've taken literally.
- **Q9 — What happens when a rerolled-away question was AI-generated fresh (not drawn from the bank pool)?** §2 as written just lets it lapse (relies on `question_items`/`fetchRecentTexts`' ordinary history to avoid exact-duplicate text, no permanent-exclusion row since there's no bank row to attach one to). If emci wants AI-generated-and-rerolled questions to also get a durable "never show this exact text again" guarantee as strong as the bank-item case, that needs its own table (e.g. keyed on a hash of the prompt text) rather than relying on the history window — flagging so this isn't silently weaker than the bank case without emci noticing.
- **Q10 — `question_bank_pool`'s ~100 target is described as a "soft cap enforced in application code," not a DB constraint.** Is ~100 a hard ceiling emci wants enforced (reject/no-op once reached even if an axis is short), or a rough steady-state target that can be exceeded if real demand needs more of one axis? This changes whether `addToBankPool` needs a total-count guard in addition to the per-axis-shortfall check.
- **Q11 — wave44's `category_question_items`/`category_question_batches` likely have live rows for users who partially completed a category batch under the current system.** Dropping the tables destroys that in-progress state with no migration path mentioned in the finalized plan. Confirm emci is OK with in-progress category-batch answers being lost outright (matching the "remove entirely, nothing left dormant" instruction), vs. wanting a one-time read/export of any in-progress rows before the drop.
