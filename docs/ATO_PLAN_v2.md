...
| `evening_wind_down` | string, nullable on pre-intake rows | Chip phrase (anchor like `morning_cue`). Flavors generated card content and personalizes the evening Check push body via `cueAfterYou`. Never times pushes (that's `energy_pattern`). Editable later in Settings. |
...
| `recovery_style` | *(removed from intake 2026-09-01 — was an orphan, no downstream effect)* | Column stays on `me` for existing users' historical values (never written from the app anymore; `complete_signup` re-save coalesces, does not overwrite). |
...
| Optional trait axes | numeric 0–1, all nullable | 16-axis backbone, separate from the 9 chips. Shipped: `openness`, `conscientiousness`, `extraversion`, `agreeableness`, `steadiness`; `attachment_anxiety`, `attachment_avoidance`; `conflict_assertiveness`, `conflict_cooperativeness`; `autonomy`, `competence`, `relatedness`; `growth_mindset`; `locus_of_control`; `self_efficacy`; **`playfulness`** (16th, Wave 21). Plus `trait_sources`: **direct** (`self_slider`, `self_tap`, `self_confirm`, `self_settings`) is sticky and cannot be overwritten by **inferred** (`self_grid`, `self_situation`, `self_game`); inferred numeric writes damp toward the signal instead of replacing; skip/null has no source row. You-tab bands (collapsed) show filled axes as a marker between two plain-language phrases — same ME columns, never a score or a name. You-tab Full Profile (private Settings fold, not the poster) lists all 16: answered or "not answered yet," last source in plain language, last updated, AxisTaps edit via `updateTraits`; completion is `N of 16 settled` (stability-weighted; gut-call never counts) on that fold and Sage. Infinite Questions live on their own screen ("Tell Sage more") off You, not a Home fold. They map multiple-choice items onto those same 16 axes; batch regen is a separate `ai_usage.by_type.questions` cap; the Explore output fence runs on question text and options; Skip this one / Skip the rest plus a soft sitting checkpoint. Per-axis `last_touched` lives in `trait_touched_at` (ISO, null axes have no key). Raw labels discarded at write. Never on poster, `peer_profile`, `public_profile`, or `night_snapshot`. AxisTaps has an 8s inline undo (mis-taps only; no persist / no history if undone). Every numeric write through `persistMergedTraits` also appends `trait_history`. Full Profile shows 2-letter codes (tap to reveal the name) and an expandable "How this has shifted" timeline (growth language, never correction). IQ offline bank covers all 16 axes; 5-item rotation is unchanged; a separate full-sweep path returns one item per axis. Optional skippable intake sweep uses that sweep — copy is drafted, **unreviewed**. Pole copy, concept explainers, and Playfulness poles are **unreviewed**. |
| `trait_history` | append-only rows | `id`, `user_id` (FK `auth.users` ON DELETE CASCADE), `axis`, `value`, `source`, `created_at`. RLS `auth.uid() = user_id`. No second writer. Cascades on self-delete and `delete_branch`. |
| `trait_tracks` | per-axis EWMA | PK `(user_id, axis, track)` where `track` is `report` or `game`. Columns: `value` (0–1, EWMA-blended, never overwritten), `stability` (0–1), `answer_count`, `last_touched`, `last_depth_at`. **self_game never blends into report value.** Completeness is stability-weighted `N of K settled` (`K` = live `TRAIT_AXES.length`). Cascades with `auth.users`. |
| `category_defs` / categories | catalog + client math | Live named categories (currently 9). Bars: Steadiness, Openness to life, Drive, Agency, Everyday social energy, Communication, **Levity** (Playfulness + conflict assertiveness + cooperativeness — a bar, not a map). Maps: Love / closeness (attachment anxiety × avoidance; conflict style is texture, not math — the only map built from conflict-adjacent axes); Independence & closeness (autonomy × relatedness). **Report-track only — never `self_game`.** Bars show once enough listed axes clear the existing stability floor. Maps are stricter: **both** axes must independently clear the floor before the map renders. Confirm-upgrade does not change category numbers. Combined title+category copy is one Gemini call on the existing title quota (`claim_title_generate`), not Talk. The Story is a **separate** call on `claim_story_generate`. Fallback bands are banked and **unreviewed**. |
| `category_share` / `close_friends_share` | dual consent | Per-friend category-card share, off by default; **both** must opt in. Close Friends pool: opt in once, visible within that group, no per-person toggling inside it. `peer_category_pack` returns cached title+category summaries only — never Full Profile. |
| `sage_title` / `sage_title_flags` | cache + capture | Live Sage title from stable report-track axes only (Explore fingerprint + 1 gen per local day). Own daily cap via `claim_title_generate` (`ai_usage.by_type.title`, default 3) — **does not increment Talk `calls`**. Pushback is insert-only; no auto-action. Copy unreviewed (`TITLE_COPY_REVIEWED = false`). |
| `sage_story` | cache | Longer-form Sage prose from currently settled categories. Own fingerprint, own quota (`claim_story_generate` / `by_type.story`, default 1/day) — **does not increment Talk or title**. Thin-profile gate uses `isThinProfile` (6/15 of live axes). No offline fallback: if the model is unreachable the section is not rendered. Never names categories inside the text. Told-vs-played tension is named with hedged language when present. **UNREVIEWED. Diagnosis-adjacent — same bar as the Crisis spec. Not shippable without emci's direct read.** |
| `tokens` | integer on ME, default 0 | Earned notes balance. Ledger is `token_events` (same cascade). Mutations only via `earn_tokens` / `spend_tokens` — never a purchase path. Earn: check-in, completing a game round, trickle (never a 0 payout). Spend, fixed prices: extra Sage look (no trait write); Full Profile Depth (real capture for a chosen axis). Tokens gate the *action* of a new capture, never viewing Full Profile. Never on Home / Check / crisis / widget. |
...
| Explore | own tab | Periodic Sage observations on a real bottom tab (`/explore` — Home / Sage / Explore / Around / Circle / You). Weekly, or on a meaningful trait/signal change — not daily, not a Home inner tab. Cached between regenerations. Existing per-user quota. Cap: **1 regeneration per calendar day**. May generate from any currently-defined category. Combines **at most 2 categories** per entry, and only when a real recent signal ties both — never a default habit. Must not restate the Categories card on the same screen that day. Never the three agency axes (`growth_mindset`, `locus_of_control`, `self_efficacy`) together. Library-grounded. Prompt includes internal category (or axis) grounding so tagged merge and prose stay aligned (never rendered). Same output fence as cards/Talk. Completeness is never an Explore input. Never empty of extra-axis data: the 9 core chips are enough. The tab holds, top to bottom: title card, Categories (full detail), The Story, Notes insight spend, then the observation bubbles. A "Did this land?" tap shows a brief local "Noted." fade — not persisted, not a model call. **Shipped Aug 29, 2026; moved into Sage Aug 30; axis grounding Aug 30; category combine + pinned-card gate Aug 31; pulled into its own screen Sep 1; became a real tab Sep 1.** |
...
Widget: Read + if-then Do. Home: face from yesterday's Check, one line in their style (lift / even / cut), one finishable Do, optional Nudge when a real signal exists, Did/Skip, then the primary slot (crisis / missed check / note / weekly Ask / Sunday week). **Deliberate Home override (Aug 31, 2026):** a second small collapsed category teaser may share the page with note / ask / week / none. It never appears next to crisis or missed-check. This reverses the original one-slot Box 8 rule on purpose — not a bug. No inner tabs. **Explore** is a real bottom tab (Home / Sage / Explore / Around / Circle / You; periodic, not daily). Evening: Check **today** (did/skip). Face moves. Result strip. Stop. Reload, when built, cycles stored variants of today's pack — it is not a new dawn.
...
- Sage: reflect more than ask, ~4 sentences, coach not doctor. Root writing rule, inherited everywhere Sage writes: *Reflect patterns as maybes, not facts. No "you are." No framework names. One trait at a time. Notice, don't correct.* Explore may combine at most 2 categories per entry **only when a real recent signal ties both**; never combine `growth_mindset` + `locus_of_control` + `self_efficacy` in one entry; the other clauses still apply. Dawn Read may draw from one settled category among Steadiness / Agency / Drive, or from knocks/facts/focus when none of those three are settled. Crisis keyword hit → static resource card (see Crisis spec below), stop, no model call.
...
- You tab = poster (name, @handle, show_up visibility label, QR, Share — no large pixel; the live face is the nav companion; name appears once, on the poster). "Tell Sage more" pushes a dedicated questions screen (not a Home fold). Weeks, then "If you need someone now" (crisis region picker), then trait bands, then Full Profile (all currently-defined axes, private). Settings: the 9 identity chips are editable (same chip UI as onboarding); talk_style + voice_preset share "How Sage sounds"; appearance / credits; timezone lives in an Account fold. Stored Teach-Sage facts have a summary row next to "How you show up" (read/delete only; teaching stays Chat "Teach Sage this"). Talk-style and voice-preset rows show a local sample line (never a model call). Milestone badges and growth bars live on You, not Home. Profile completeness is **two separate things**: the 9 core chips are always complete (never shown as a percentage of a person); the currently-defined axes are a separate depth layer, invitation not deficit (`N of K settled` from `TRAIT_AXES.length` on the Full Profile fold and Sage). No completeness signal on Home, widget, or push. Explore prompt still does not take completeness as an input. Sage reply room is shown as `X of [limit]` (e.g. `6 of 20 today`) with no "AI" or "tokens" in the copy — compact on Sage, collapsed by default in Settings. The crisis-line region picker is "If you need someone now" after Weeks; it is not the active Talk crisis card. Sage opens that same card from an always-visible Support tap near the composer (no keyword; also when Talk is off). Sage is a permanent un-hideable tab. AI consent is a Dawn/Sage interstitial (first answer) plus a You Account row ("Sage's AI") afterward.
...
- Home may show a third daily category, **Nudge**, from a real recent signal only. Empty when there isn't one. Never Circle, widget, or morning push. **Explore** is a real bottom tab (Home / Sage / Explore / Around / Circle / You), not a Home inner tab and not on the You tab. Completeness is never an Explore input. Explore inherits the same output fence; regen cap is 1 per calendar day. Generation is grounded to the already-picked axis inside `buildExplorePrompt` (prompt scaffolding only — never shown).
...
- Model provider: one env var (`EXPO_PUBLIC_AI_PROVIDER`) selects Gemini / NVIDIA / Perplexity / Claude / Grok / local. Claude and Grok are called from the `ai-generate` Edge Function so their keys never ship in the client. `MODEL_PROVIDER=local` still forces the deterministic fallback. Don't hardcode a vendor name into Dawn or Talk.
...
| Box | In | Out |
|---|---|---|
| Home | card | Check **today** (did/skip) + result strip. Optional Nudge. Primary slot below Did/Skip, plus a collapsed category teaser when the slot is not crisis/missed-check (deliberate Aug 31, 2026 override of the one-slot rule). Explore is a real bottom tab (periodic). Pixel is the global nav companion, not inline on Home. |
...
2. **Response**: a static, non-AI-generated card. Region is detected from device locale/timezone at launch (manual override is "If you need someone now" on You after Weeks — a region picker, not the active Talk card). Confirmed numbers only: US (`988 Suicide & Crisis Lifeline`) and Canada (`988 Suicide Crisis Helpline`) — call or text 988. Any other region gets an honest fallback, never a guessed number. The card also makes clear Sage is a coach, not a person who can help in an emergency. Sage has an always-visible Support tap (under the composer; also when Talk is off) that opens this same card with no keyword. The keyword interrupt path is separate and unchanged.
...
- **Inferred**: 16-grid (`self_grid`), situation tap (`self_situation`), game/swipe (`self_game`). Numeric writes are damped (`0.12 * signal + 0.88 * old`; null old uses 0.5). Direct sources still full-replace. Confirm-upgrade never changes the number.
...
1. **Shipped (Stage 11):** skippable `extra N of 9` after signup succeeds — type grid + 8 vibe-check scenarios (same nine trait axes as the old grid/slider/close/disagree layer). Skip = those columns stay null. Merge is **direct vs inferred** (slider / tap / confirm / Settings sit in the direct rank and full-replace; inferred damps toward the signal). "Love language" reframed platonic remains a lower-priority optional add, not Chapman categories.
...
Exception: **Explore** may combine at most 2 categories per entry, and only when a real recent signal ties both — never a default habit. Never combine the three agency axes in one entry. Same no-framework-names and exploratory-language rules still apply. This replaces per-feature prompt instructions.
...
### Explore (own tab — shipped Aug 29, 2026; moved off Home Aug 30; category combine Aug 31; pulled out of Sage Sep 1; became a real tab Sep 1)

Not on the You tab. Not a Home inner tab. Not stacked in the Sage chat thread — Explore is a real bottom tab (`/explore`). The Sage tab is clean chat only. The bottom bar is a custom JS `NavOrder` bar (Home/Sage pinned and swappable; More is the fixed rightmost slot; Explore / Around / You / Circle reorderable). Gated tabs (Circle until a friend is scanned) leave the bar and More and show only under "Not unlocked yet" in Edit Navigation. Periodic generation only (weekly, or on a meaningful trait/signal change) — never daily. Cached between regenerations. Uses the existing per-user quota. **Cap: 1 regeneration per calendar day** (a swipe-deck binge batches into one rebuild after the session, not one per axis). The tab holds, top to bottom: title card, Categories (full detail), The Story fold, Notes insight spend, then generated observations. After a category (or fallback axis) is picked, `buildExplorePrompt` feeds an internal grounding line (prompt scaffolding only — never rendered) so the observation matches the tagged merge.

May generate from any currently-defined category. Combines **at most 2 categories** per entry, grounded in the Library, **only when a real recent signal ties both** — never a default habit. No recent signal → stay at one category, or (if none are ready) one trait or the 9 chips. Must not restate the pinned Categories card from the same screen that day (prompt + topical overlap gate). **Never combine** `growth_mindset`, `locus_of_control`, and `self_efficacy` in one entry (they are one agency idea — which-three, not a total). Completeness is **never** an Explore input: no remaining-gap copy, no "richer because they filled the bar."
...
### Profile completeness indicator (You tab)
...
- The 15 axes are a separate **depth** layer, framed as invitation not deficit. Completeness is stability-weighted report-track only — gut-call never counts toward "settled."

**Shipped:** You-tab Full Profile fold (auth tab only, not poster / `@handle`). All currently-defined axes: phrase-band or "not answered yet"; last source in plain language (told / gut-call inferred / Sage-knows correction / still-fits); last updated from `trait_touched_at` / track `last_touched`; AxisTaps edit through existing `updateTraits` (`self_tap` first fill, `self_settings` later) with an 8s undo (cannot undo the same axis twice in a row); 2-letter code tap-to-name; expandable "How this has shifted" from `trait_history`; draft pole descriptions (unreviewed). Fold title shows `N of K settled` (stability-weighted report-track; `K` = live `TRAIT_AXES.length`; floor of 3 answers before stability reads as meaningful; 60-day idle then 90-day half-life decay at read). Sage tab shows the same number. Thin-profile Talk coaching fires below 6/15 of that live total. Pressure-free: no red/urgent framing, no streak, no guilt copy. **No completeness signal on Home, widget, or push.** Do not wire it into pixel depth glow (`facts` stays that axis). Tokens can trigger a new capture (Depth, 48h per-axis cooldown); they never hide an existing reading. Live Sage title + one-line from stable axes only; user can flag "this doesn't feel right" (capture only).

**Still later:** You/Settings weekly completeness slot vs 3-month re-ask (soft-ask budget); gap tap into a scenario card. Depth count does not claim that weekly slot.
...
**Where this fits:** Stages 9, 11, and 12 shipped (intake, axes, Library grounding). Talk output fence shipped. Stage 13 badges, Does-Sage-know-you, reveal, ranking, and scenarios shipped. Explore (Sage thread + phrasing-only feedback + axis grounding) shipped. IA reorganization (one Ask, Tell Sage more, Home one-slot, You regroup, consent interstitial, support promoted, honest-empty Today) shipped. Remaining work is later boxes (3-month Settings prompt, You-tab weekly completeness slot, three-path extra-axis intake chooser, Reload) plus emci review of intake/IQ sweep copy — not one box. Locks from the Aug 28 Grok review live in the sections above.
...
- **Forced-ranking sort** — shipped. Weekly Ask: drag 4–5 short statements into order from "most me" to "least me." One axis per round (first nine), plain behavioral lines, `self_tap` write. Soft-ask: yields if Does-Sage-know-you or completeness already has the week; extra-axis gaps go to scenarios. Standalone compare-two pick covers all 15 (including the extra six) for intake Depth / token spend — does not claim the weekly slot, not merged into IQ.
- **Scenario reaction cards** — shipped. Weekly Ask: swipe or tap, two options, one extra axis per card. SDT is three cards, never a three-way pick. Writes `self_game`. Same weekly slot. Standalone / token Depth can use a second stem when that axis already has a reading. Stays a distinct inferred signal (needed for Sage divergence).
- **Notes (earned tokens)** — shipped. Balance on `me.tokens`, ledger `token_events`. Earn only (check-in / game round / trickle; never 0). Spend at a known price: extra Sage look (no trait write) or Full Profile Depth (real IQ/ranking/scenario capture). No purchase path. Does not paywall Home, Check, crisis, widget, Talk, or viewing Full Profile.
- **Sage titles** — shipped as live-generated (not a lookup) from stable report-track axes only. Unstable axes excluded. Explore-style fingerprint + 1 gen per local day. Own `claim_title_generate` cap (does not increment Talk `calls`). User can flag "this doesn't feel right" (stored, no auto-action). Copy unreviewed (`docs/wave20-copy-DRAFT.md`). **Age-sensitivity:** a personality-type-style label sits next to the same 16+ / loot-box proximity concern already on record — flagged in BUSINESS.md for legal/copy review; no extra code.
- **Optional intake sweep** — shipped as skippable. One unanswered IQ item per axis, all 15, each skippable. Never required to finish onboarding. Copy in `docs/intake-sweep-copy-DRAFT.md` is **unreviewed** (`INTAKE_SWEEP_COPY_REVIEWED = false`).
...
- `ai_usage` table: log calls per user per day. A separate lightweight `ai_provider_log` (provider + timestamp only) powers the hidden `/ai-lab` switcher's rolling counts.
- Config-driven per-user daily/monthly cap (`claim_ai_call` and typed claim RPCs).
- **Shipped Sep 1, 2026:** one active vendor via `EXPO_PUBLIC_AI_PROVIDER` (on-device override possible). Gemini / NVIDIA / Perplexity client-side; Claude / Grok via `ai-generate`. Not yet a waterfall: primary fails or user hits quota does **not** automatically try a second vendor. Honest empty when the selected provider fails or quota is spent. Moving remaining client keys off-device is still required before `signup_mode` flips to `public`.
...
`check_count < 3` → bank (`first_cards.md`). `check_count >= 3` → model, via router, provider set by `EXPO_PUBLIC_AI_PROVIDER` (`MODEL_PROVIDER=local` still forces the deterministic fallback). Drop repeats, vague Dos, cruel cuts before showing anything.
...
Apple Sign In: hide-my-email maps to exactly one user, no fork. Bundle ID `com.emgens.ato` (App ID) / `com.emgens.ato.signin` (Services ID). Edge Function `APPLE_CLIENT_ID` for native code exchange must be the **bundle ID** (`com.emgens.ato`), not the Services ID. Email OTP + Apple stay the primary auth paths on separate Sign up and Log in screens. Password is optional: set/change in You Settings after OTP or Apple (GoTrue bcrypt on `auth.users`); Log in accepts email/@handle + password; OTP still works if they never set one. No password field on Sign up.
...
**Decided Aug 28, 2026 — later boxes, not Stage 12, not one combined box:** Explore (Sage thread) + phrasing-only feedback **shipped Aug 29, 2026** (moved off Home Aug 30; axis grounding Aug 30); 3-month Settings prompt; intake three-path for the extra axes (core 9 unchanged); You-tab weekly completeness slot; Dawn Reload with the locks already closed. **You-tab Full Profile + `N of 15 settled` shipped.** Talk output fence **shipped**. Six extra trait axes + direct-vs-inferred `trait_sources` + `last_touched` **shipped**. Library copy **shipped**. Sage reads Library For Sage lines **shipped** (Stage 12). Stage 13 badges **shipped** (7 Checks / first fact / week without a cut). Does-Sage-know-you **shipped** (banked check-in). Home reveal **shipped**. Forced ranking **shipped**. Scenario swipe-deck **shipped**. **IA reorganization (15 boxes) shipped Aug 30, 2026** — one weekly Ask, questions on "Tell Sage more", Home one-slot, You regroup, consent interstitial, support promoted, honest-empty Today. **Locks from the Aug 28 Grok review are in the Understanding spec** (Explore combine + regen cap + fence; Does-Sage-know-you confirm rules; completeness split; direct-vs-inferred `trait_sources`; Talk fence phrases + retry-is-not-quota; soft-ask budget). Do not fold remaining items into Stage 12.
...
**Open box: intake, dawn.**
Reveal card (real content only, no randomized-value mechanic), milestone badges, "Does Sage know you?" active-learning loop (confirm/correct + 3-month Settings prompt; locks in Understanding spec), forced-ranking sort, scenario reaction cards with light time pressure. Explore is Sage-thread entries (Understanding spec), not this box's reveal card. Soft-ask budget: at most one optional-depth prompt visible at a time. After the IA pass: badges/growth live on You; Does-Sage-know-you / ranking / Gut call share one weekly Home Ask slot; questions are "Tell Sage more" off You.
...
**Paid tier.** Unlocks after 7 Checks: weekly Read, 30-day trail, more Talk. Never paywall Home, More, Check, crisis response, or the widget. In-app notes (`me.tokens`) are a separate earned layer — never a purchase path, never a substitute for this paid tier.
