...
| `trait_tracks` | per-axis EWMA | PK `(user_id, axis, track)` where `track` is `report` or `game`. Columns: `value` (0–1, EWMA-blended, never overwritten), `stability` (0–1), `answer_count`, `last_touched`, `last_depth_at`. **self_game never blends into report value.** Completeness is stability-weighted `N of 15 settled`. Cascades with `auth.users`. |
...
Widget: Read + if-then Do. Home: face from yesterday's Check, one line in their style (lift / even / cut), one finishable Do, optional Nudge when a real signal exists, Did/Skip, then at most one slot below (crisis / missed check / note / weekly Ask / Sunday week — never two). No inner tabs. **Explore** lives in the Sage thread (periodic, not daily). Evening: Check **today** (did/skip). Face moves. Result strip. Stop. Reload, when built, cycles stored variants of today's pack — it is not a new dawn.
...
| Box | In | Out |
|---|---|---|
| Home | card | Check **today** (did/skip) + result strip. Optional Nudge. At most one slot below Did/Skip. Explore is in the Sage thread (periodic). Pixel is the global nav companion, not inline on Home. |
