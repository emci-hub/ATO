...
- The 15 axes are a separate **depth** layer, framed as invitation not deficit. Inferred-from-game data **counts as filled**.

**Shipped:** You-tab Full Profile fold (auth tab only, not poster / `@handle`). All 15 axes: phrase-band or "not answered yet"; last source in plain language (told / gut-call inferred / Sage-knows correction / still-fits); last updated from `trait_touched_at`; AxisTaps edit through existing `updateTraits` (`self_tap` first fill, `self_settings` later) with an 8s undo; 2-letter code tap-to-name; expandable "How this has shifted" from `trait_history`. Fold title shows `N of 15 answered`. Sage tab shows the same count. Pressure-free: no red/urgent framing, no streak, no guilt copy. **No completeness signal on Home, widget, or push.** Do not wire it into pixel depth glow (`facts` stays that axis). Tokens can trigger a new capture (Depth); they never hide an existing reading.
...
Delete account **in-app** + revoke Sign in with Apple token. Ask about notifications once, after the user's first card exists — app works fully if they say no. Re-verified Aug 28, 2026: ME is hard-deleted (no flag); owned rows cascade including later tables (`ai_usage`, `going`, Stage 11 axes on ME, `trait_history`, `token_events`); `account_deletions` is the surviving audit row (no FK). Apple proof is `confirmRevoked` (refresh token no longer grants), not `/auth/revoke` 200.
