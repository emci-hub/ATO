...
| `sage_story` | cache | Longer-form Sage prose from currently settled categories. Own fingerprint, own quota (`claim_story_generate` / `by_type.story`, default 1/day) — **does not increment Talk or title**. Thin-profile gate uses `isThinProfile` (6/15 of live axes). No offline fallback: if Gemini is unreachable the section is not rendered. Never names categories inside the text. Told-vs-played tension is named with hedged language when present. **UNREVIEWED. Diagnosis-adjacent — same bar as the Crisis spec. Not shippable without emci's direct read.** |
