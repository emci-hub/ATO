...
| Explore | Sage thread entries | Periodic Sage observations at the top of the Sage thread (weekly, or on a meaningful trait/signal change) — not daily, not a Home inner tab. Cached between regenerations. Existing per-user quota. Cap: **1 regeneration per calendar day**. May generate from any currently-defined category. Combines **at most 2 categories** per entry, and only when a real recent signal ties both — never a default habit. Must not restate the pinned Categories card on the same screen that day. Never the three agency axes (`growth_mindset`, `locus_of_control`, `self_efficacy`) together. Library-grounded. Prompt includes internal category (or axis) grounding so tagged merge and prose stay aligned (never rendered). Same output fence as cards/Talk. Completeness is never an Explore input. Never empty of extra-axis data: the 9 core chips are enough. The Story fold sits below the pinned Categories card; generated observations sit below that. A "Did this land?" tap shows a brief local "Noted." fade — not persisted, not a model call. **Shipped Aug 29, 2026; moved into Sage Aug 30; axis grounding Aug 30; category combine + pinned-card gate Aug 31.** |
...
- Home may show a third daily category, **Nudge**, from a real recent signal only. Empty when there isn't one. Never Circle, widget, or morning push. **Explore** is unprompted entries at the top of the Sage thread, not a Home inner tab and not on the You tab. Completeness is never an Explore input. Explore inherits the same output fence; regen cap is 1 per calendar day. Generation is grounded to the already-picked axis inside `buildExplorePrompt` (prompt scaffolding only — never shown).
...
### Explore (Sage thread — shipped Aug 29, 2026; moved off Home Aug 30; category combine Aug 31)

Not on the You tab. Not a Home inner tab. Unprompted entries at the top of the Sage thread, above the first Talk message. Separate from daily Read/Do/Nudge. Periodic generation only (weekly, or on a meaningful trait/signal change) — never daily. Cached between regenerations. Uses the existing per-user quota. **Cap: 1 regeneration per calendar day** (a swipe-deck binge batches into one rebuild after the session, not one per axis). A pinned Categories card sits at the top of this block; The Story fold sits under it; generated observations sit under that. After a category (or fallback axis) is picked, `buildExplorePrompt` feeds an internal grounding line (prompt scaffolding only — never rendered) so the observation matches the tagged merge.
