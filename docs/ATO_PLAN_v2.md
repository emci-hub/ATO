...
| `category_defs` / categories | catalog + client math | 8 named categories. Bars: Steadiness, Openness to life, Drive, Agency, Everyday social energy, Communication. Maps: Love / closeness (attachment anxiety × avoidance; conflict style is texture, not math); Independence & closeness (autonomy × relatedness). **Report-track only — never `self_game`.** Bars show once enough listed axes clear the existing stability floor. Maps are stricter: **both** axes must independently clear the floor before the map renders. Confirm-upgrade does not change category numbers. Combined title+category copy is one Gemini call on the title quota (`claim_title_generate`), not Talk. Fallback bands are banked and **unreviewed**. |
...
| Explore | Sage thread entries | Periodic Sage observations at the top of the Sage thread (weekly, or on a meaningful trait/signal change) — not daily, not a Home inner tab. Cached between regenerations. Existing per-user quota. Cap: **1 regeneration per calendar day**. Combines 2–3 traits only when at least one is tied to a recent signal (fact / knock / Check pattern); never the three agency axes (`growth_mindset`, `locus_of_control`, `self_efficacy`) together. Library-grounded. Prompt includes a per-axis internal grounding line so tagged axis and prose stay aligned (never rendered, never a UI string). Same output fence as cards/Talk. Completeness is never an Explore input. Never empty of extra-axis data: the 9 core chips are enough. Separate from Read/Do/Nudge and from the You tab. A "Did this land?" tap shows a brief local "Noted." fade — not persisted, not a model call. **Shipped Aug 29, 2026; moved into Sage Aug 30; axis grounding Aug 30.** |
