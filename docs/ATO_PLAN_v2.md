...
| Explore | own full screen | Periodic Sage observations on a dedicated `/explore` screen (pushed from a small "Explore ›" button in the Sage header — not a nav tab, not stacked above the Sage chat). Weekly, or on a meaningful trait/signal change — not daily, not a Home inner tab. Cached between regenerations. Existing per-user quota. Cap: **1 regeneration per calendar day**. May generate from any currently-defined category. Combines **at most 2 categories** per entry, and only when a real recent signal ties both — never a default habit. Must not restate the Categories card on the same screen that day. Never the three agency axes (`growth_mindset`, `locus_of_control`, `self_efficacy`) together. Library-grounded. Prompt includes internal category (or axis) grounding so tagged merge and prose stay aligned (never rendered). Same output fence as cards/Talk. Completeness is never an Explore input. Never empty of extra-axis data: the 9 core chips are enough. The screen holds, top to bottom: title card, Categories (full detail), The Story, Notes insight spend, then the observation bubbles. A "Did this land?" tap shows a brief local "Noted." fade — not persisted, not a model call. **Shipped Aug 29, 2026; moved into Sage Aug 30; axis grounding Aug 30; category combine + pinned-card gate Aug 31; pulled into its own screen Sep 1.** |
...
- Home may show a third daily category, **Nudge**, from a real recent signal only. Empty when there isn't one. Never Circle, widget, or morning push. **Explore** is its own full screen (pushed from a Sage header button), not a Home inner tab and not on the You tab. Completeness is never an Explore input. Explore inherits the same output fence; regen cap is 1 per calendar day. Generation is grounded to the already-picked axis inside `buildExplorePrompt` (prompt scaffolding only — never shown).
...
### Explore (own screen — shipped Aug 29, 2026; moved off Home Aug 30; category combine Aug 31; pulled out of Sage Sep 1)

Not on the You tab. Not a Home inner tab. Not stacked in the Sage chat thread — Explore is its own full screen (`/explore`) opened from a small "Explore ›" button in the Sage header (not a nav tab; the tab bar stays Home / Sage / Around / Circle / You). The Sage tab is clean chat only. Periodic generation only (weekly, or on a meaningful trait/signal change) — never daily. Cached between regenerations. Uses the existing per-user quota. **Cap: 1 regeneration per calendar day** (a swipe-deck binge batches into one rebuild after the session, not one per axis). The screen holds, top to bottom: title card, Categories (full detail), The Story fold, Notes insight spend, then generated observations. After a category (or fallback axis) is picked, `buildExplorePrompt` feeds an internal grounding line (prompt scaffolding only — never rendered) so the observation matches the tagged merge.
