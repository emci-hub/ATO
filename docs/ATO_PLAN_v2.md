...
Widget = Read + Do. Morning push = Read. Evening push = Check today. Sunday push = `this_week` recap + "you showed up N." Deep links from push into the right screen. **Push + widget is built and device-verified** (local morning/evening/Sunday, iOS WidgetKit Read+Do, App Group `group.com.emgens.ato`). **Floor requirements in this box are built and re-verified:** Sentry wired (`ato-app`; live JS ingest this session; native crash still needs the next EAS build), App Privacy nutrition labels aligned with `privacy.md` + `PrivacyInfo.xcprivacy` (10 types, `NSPrivacyTracking = false`), Sage labeled coach in the UI (Talk/Home/Dawn/consent/crisis/morning push/widget/Teach Sage), Talk router rate-limited per user server-side (`claim_ai_call`, live 20/day 200/month). Remaining in this box: landing polish if needed, then TestFlight submit.
...
- [x] Sentry DSN connected (`EXPO_PUBLIC_SENTRY_DSN` in `.env.local` and EAS) and a test JS error confirmed in Issues (`ato-app`). Native crash still needs the next EAS / TestFlight binary.
