...
| `trait_tracks` | per-axis EWMA | PK `(user_id, axis, track)` where `track` is `report` or `game`. Columns: `value` (0–1, EWMA-blended, never overwritten), `stability` (0–1), `answer_count`, `last_touched`, `last_depth_at`. **self_game never blends into report value.** Completeness is stability-weighted `N of 16 settled`. Cascades with `auth.users`. |
...
**Shipped:** You-tab Full Profile fold (auth tab only, not poster / `@handle`). All 15 axes: phrase-band or "not answered yet"; last source in plain language (told / gut-call inferred / Sage-knows correction / still-fits); last updated from `trait_touched_at` / track `last_touched`; AxisTaps edit through existing `updateTraits` (`self_tap` first fill, `self_settings` later) with an 8s undo (cannot undo the same axis twice in a row); 2-letter code tap-to-name; expandable "How this has shifted" from `trait_history`; draft pole descriptions (unreviewed). Fold title shows `N of 15 settled` (stability-weighted report-track; floor of 3 answers before stability reads as meaningful; 60-day idle then 90-day half-life decay at read). Sage tab shows the same number. Pressure-free: no red/urgent framing, no streak, no guilt copy. **No completeness signal on Home, widget, or push.** Do not wire it into pixel depth glow (`facts` stays that axis). Tokens can trigger a new capture (Depth, 48h per-axis cooldown); they never hide an existing reading. Live Sage title + one-line from stable axes only; user can flag "this doesn't feel right" (capture only).
