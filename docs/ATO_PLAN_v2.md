...
**Shipped:** You-tab Full Profile fold (auth tab only, not poster / `@handle`). All 15 axes: phrase-band or "not answered yet"; last source in plain language (told / gut-call inferred / Sage-knows correction / still-fits); last updated from `trait_touched_at`; AxisTaps edit through existing `updateTraits` (`self_tap` first fill, `self_settings` later). Fold title shows `N of 15 answered`. Pressure-free: no red/urgent framing, no streak, no guilt copy. **No completeness signal on Home, Explore, Talk, widget, or push.** Do not wire it into pixel depth glow (`facts` stays that axis).
