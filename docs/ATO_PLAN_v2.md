...
**Where this fits:** Stages 9, 11, and 12 shipped (intake, axes, Library grounding). Talk output fence shipped. Stage 13 badges, Does-Sage-know-you, and reveal shipped. Remaining work is several later boxes (Explore, feedback, 3-month Settings prompt, completeness, three-path extra-axis intake, Reload, ranking, scenarios) — not one box. Locks from the Aug 28 Grok review live in the sections above.
...
**Done (Aug 28, 2026, reveal):** Home tap-to-open Note. Week pattern / fact / badge-proximity, Nudge-style priority. One 300ms unfold, same motion for every kind. Reduce Motion skips to copy. Empty days are a plain line, no sealed object. Ranking and scenarios remain later.
